...
def affine_forward(x, w, b):
    ...
    pass
    M = x[0].shape
    X = x.reshape(x.shape[0], np.prod(M))
    out = X.dot(w) + b
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #calculating the hight and width of the activation map
    activation_map_hight = 1 + (H + 2 * pad - HH) / stride
    activation_map_width = 1 + (W + 2 * pad - WW) / stride
    out = np.zeros((N, F,activation_map_hight,activation_map_width))
    
    #padding each input 
    for i in xrange(N):
        x_temp = np.pad(x[i], [(0, 0), (pad, pad), (pad, pad)], 'constant')

        for j in xrange(activation_map_hight):
            for k in xrange(activation_map_width):
                # getting the indices of the square that will be multiplied by the filter 
                h_start = j * stride
                h_end = h_start + HH
                ...
                # multiplying the scuare by each one of the filters
                for f in xrange(F):
    ...
    for i in xrange(N):
        ...
        for j in xrange(H_prime):
            for k in xrange(W_prime):
                ...
                local_receptive_field = x_temp[:, h_start:h_end, w_start:w_end]
                # couldn't understand ):
                # maybe: getting the gradient for each square and adding to the Filter
                # which means every square wil afect the Filter the same way
                for f in xrange(F):
    ...
    for i in xrange(N):
        for j in xrange(H_prime):
            ...
            for c in xrange(C):
                ...
                # finding who was themax val in the square (the only ine who affects the forward procces 
                # so he will be the only one who's derivative wont be 0 
